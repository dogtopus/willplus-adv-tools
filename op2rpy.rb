...
module RIOASMTranslator
    ...
    def translate(scr_name, scr)
        ...
        @array_table = []
        FLAG_TABLE.each do |key, val|
            array_size = val[3]
            @array_table << [key, key + array_size - 1] unless array_size.nil?
        end
    ...
    def _get_flag_reference(flag_addr, on_hint)
        ...
        if v.nil?
            # Look for array membership
            @array_table.each do |range|
                if flag_addr.between?(range[0], range[1])
                    v = FLAG_TABLE[range[0]]
                    offset = flag_addr - range[0]
                    break
                end
            end
        end
        ...
        if v
            ...
            offset_fmt = (offset.nil?) ? nil : ", #{offset}"
            flag_ref = "#{bank}[#{(v[0].nil?) ? flag_addr : ("'#{v[0]}'")}#{offset_fmt}]"
            if v[1] == Flag::EXCLUDE
                return nil
            elsif v[1] == Flag::HINT
    ...
    def op_set(operator, lvar, reference_level, rside, try_boolify=false)
        ...
        @rpy.add_cmd("$ #{flag_ref} #{operator} #{rside}") unless flag_ref.nil? || rside.nil?
    end

    def op_mov(lvar, is_flag, rside)
        op_set('=', lvar, is_flag != 0 ? 1 : 0, rside, true)
        # typewriter effect
